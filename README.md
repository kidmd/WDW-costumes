...
* **Costume Simulation:** Visual browser-based layout engine (`python simulator.py` @ `localhost:8000`) with built-in presets (Pete's Dragon, Cinderella's Coach, Carriage No Horses, Casey Jr., Title Drum, Turtle, Snail, Eagle), 7-Shirt Fleet Lineup Manager with real 700-LED mini-shirt visualizer, choreographed 20-second parade routine (1s Blackout ➔ 1s Forward Color Wave with ~2-shirt trail ➔ 1s Reverse Wave with locked standard color & matching trail ➔ 5s All-Fleet Wave Color Pulse ➔ 2s Color+White Sparkle Storm ➔ 1s Blackout ➔ 9s Presets with non-repeating color rotation), 1-click & double-click Single View editing from fleet cards, multi-mode wireless sync (ESP-NOW Wave, Free-Run, Show Sequence), Fireworks Starburst cluster generator with corner placement, move & scale controls, canvas multi-drag, full-graphic LED re-distribution (maintaining strictly 100 LEDs per shirt), and authentic runDisney 10K Race Bib (#1952) with Chip & Dale and BibBoards fastener clearance.
